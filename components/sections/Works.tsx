...
import { images  } from "@/public/assets/images"; // تأكد من مسار الصور في ملف images.ts
...
export default function Works() {
  ...
  // بيانات المشاريع مرتبطة بالترجمة
  const projects = [
    {
      id: 1,
      category: "web",
    ...
    <section id="works" className="py-24 bg-slate-50/30">
      <Container>
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <h2 className={`text-4xl md:text-6xl font-black text-slate-900 mb-6 ${locale === 'ar' ? 'font-arabic' : 'font-sans'}`}>
            {t("title")}
          </h2>
              ...
              <div className="p-8 pt-4 flex flex-col flex-1">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tools.map((tool, index) => (
                    <span key={index} className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 bg-slate-50 text-slate-400 rounded-lg group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors">
                      {tool}
                    </span>
                  ))}
                </div>

                <h3 className={`text-2xl font-bold text-slate-900 mb-3 group-hover:text-emerald-600 transition-colors duration-300 ${locale === 'ar' ? 'font-arabic' : 'font-sans'}`}>
                  {project.title}
                </h3>
